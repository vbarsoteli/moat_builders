<?php
	...
	*/
	...
	if ($_SERVER['HTTP_HOST'] == '127.0.0.1' or $_SERVER['HTTP_HOST'] == 'localhost') {
		...
		// CSS
		if ($opendir = opendir('comum/css/')) {
		
			//
			while (false !== ($arquivo = readdir($opendir)))
				if ($arquivo != '.' and $arquivo != '..' and !stristr($arquivo, '.min.css'))
					$css .= file_get_contents('comum/css/'.$arquivo);
		...
		//
		file_put_contents('comum/css/geral.min.css', minify_css($css));
